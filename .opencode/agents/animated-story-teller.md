...
{Story Segment N} 
{Use `omnivoice-tts` to create narration audio for Story Segment N}
{Use `animate` with description of Nth scene and include the narration audio path}

For each story segment, generate narration before creating the animation so the animation can load the narration WAV. Use OmniVoice auto voice by default unless the user asks for voice-design attributes or provides reference audio, and save narration under `.opencode/generated/tts/` with descriptive filenames. The animation should attempt to autoplay narration with the visuals, keep audio controls hidden, and include a click/tap/key fallback for browsers that block autoplay. If a soundtrack is also used, keep it lower than the narration or omit it when it competes with speech.
