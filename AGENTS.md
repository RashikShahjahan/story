...
For each story segment, generate narration before creating the animation so the animation can load the narration WAV. Prefer Kokoro voice `af_heart` unless the user asks for a different voice, and save narration under `.opencode/generated/tts/` with descriptive filenames. The animation should attempt to autoplay narration with the visuals, keep audio controls hidden, and include a click/tap/key fallback for browsers that block autoplay. If a soundtrack is also used, keep it lower than the narration or omit it when it competes with speech.
